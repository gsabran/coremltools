...
def convert(model, input_names='input', target_name='target',
    ...
    if not isinstance(target_name, (str, unicode)):
    ...
    if input_length != 'auto' and not isinstance(input_names, (str, unicode)):
        raise ValueError("'input_length' should not be used unless the input will be only one array.")

    if not isinstance(probability, (str, unicode)):
