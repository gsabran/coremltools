    ...
    def defuse_activation(self):
        ...
        while idx < nb_layers:
            ...
            if (isinstance(k_layer, _keras.layers.convolutional.Convolution2D) or 
                ...
                isinstance(k_layer, _keras.layers.core.Dense)) and \
                k_layer.activation.func_name != 'linear':
                # Create new layer
                new_layer = layer + '__activation__'
                new_keras_layer = _keras.layers.core.Activation(k_layer.activation.func_name)
    ...
    def _get_1d_interface_edges(self):
        ...
        for layer in self.layer_list: 
            ...
            if len(preds) == 0:
                ...
                # because 1D layers are all 1-in-1-out, there should only be 1 predecessor
                pred = preds[0]
                if self.is_activation(pred): # assume activation layer is almost preceded by a dense/conv/lstm layer
                    pred = self.get_predecessors(pred)
                if not self.is_1d_layer(pred):
                    in_edges.append((preds[0], layer))
            # otherwise it's not a 1D start layer
