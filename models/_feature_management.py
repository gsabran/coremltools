...
def process_or_validate_features(features, num_dimensions = None, feature_type_map = {}):
    ...
    *   A single string.  In this case, the input is assumed to be a single array, 
    ...
    for k, v in list(features.iteritems()):
        ...
        if isinstance(v, (tuple, list, set, xrange)):
            for idx in v:
                test_index(idx)
    ...
    if num_dimensions is None:
        ...
        num_dimensions = 1 + max(chain(*[il for k, il in features.iteritems()]))

    if (set().union(*features.values()) != set(range(num_dimensions))
        or sum(len(v) for v in features.itervalues()) != num_dimensions):
    ...
    for i, (k, v) in enumerate(sorted(features.iteritems(), key = lambda t: t[1][0])):
